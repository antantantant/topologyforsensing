...
% Set parameters
nelx = 40; % horizontal number of elements (left to right)
...
observability_set = zeros(nsamples,1);
for sample = 1:nsamples
    xPhys = reshape(x(sample,:),nely,nelx);
    ...
    D = S*inv(Mb)*Sp;
    observability_set(sample) = log(det(D'*D));
    
    % calculate compliance
    Fb = sparse(find(sum(Sp,2)==1),1,-1,p,1);
    u = Kb\Fb;
    compliance_set(sample) = u'*Kb*u;
end
plot(-observability_set, log(compliance_set), '.', 'MarkerSize', 10);
save(['pareto_nx',num2str(nelx),'_ny',num2str(nely),...
    'vf',num2str(volfrac),'.mat'], 'x','observability_set','compliance_set');
